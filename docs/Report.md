...
- **Trần Yến Phượng**: Khảo sát yêu cầu, phân tích bài toán và thiết kế kiến trúc hệ thống. Chịu trách nhiệm chính về toàn bộ logic nghiệp vụ cốt lõi và hạ tầng dữ liệu (Back-end). Trực tiếp hiện thực và quản lý các thành phần sau trên Git:
  * **Kiến trúc sư hệ thống & Core Engine**: Chủ trì thiết kế kiến trúc phân tầng và luồng dữ liệu toàn hệ thống.
  * **Domain Layer**: Thiết kế và hiện thực `Entity.cs` (hệ thống thực thể), `ValueObject.cs` (đối tượng giá trị) và `Interface.cs` (hợp đồng hệ thống) đảm bảo tính trừu tượng cao.
  * **Hệ thống Quản lý Trạng thái (State Engine)**: Thiết kế và cài đặt State Pattern phức tạp cho vòng đời chuyến đi và tài xế trong `Pattern.cs`.
  * **Thuật toán & Dịch vụ Nghiệp vụ**: Phát triển các thuật toán cốt lõi trong `Service.cs` bao gồm: Ghép tài xế tối ưu (Spatial Indexing), tính toán giá cước đa tiêu chí, quản lý ví điện tử và thanh toán.
  * **Hạ tầng Dữ liệu (Infrastructure)**: Thiết kế cơ chế lưu trữ JSON Generic, xử lý đa hình (Polymorphism) trong Serialization và đảm bảo an toàn đa luồng (Thread-safety) trong `Repository.cs`.
  * **Quản lý Dự án & Git**: Cấu hình hệ thống (`.csproj`, `Program.cs`), quản lý các gói NuGet và kiểm soát xung đột mã nguồn trên Git.

- **Lê Tô Nguyệt Minh**: Thiết kế và hiện thực toàn bộ hệ thống tương tác người dùng và tích hợp dịch vụ ngoại vi (Front-end & Integration). Chịu trách nhiệm quản lý và đẩy mã nguồn lên Git cho các thành phần sau:
  * **Thiết kế Trải nghiệm Người dùng (UX/UI)**: Hiện thực hóa toàn bộ hệ thống giao diện WinForms (`Form/`) và các thành phần UI tương tác cao (`UserControl/`).
  * **Tích hợp Dịch vụ Ngoại vi**: Xây dựng module tích hợp API bản đồ thời gian thực (OSRM) và tìm kiếm địa chỉ (Photon), xử lý bất đồng bộ trong hiển thị bản đồ.
  * **Vận hành & Kiểm thử**: Phát triển module mô phỏng kịch bản (`Simulation.cs`), bộ tạo dữ liệu mẫu (`DataSeeder.cs`) và các tiện ích hỗ trợ giao diện (`UIHelper.cs`).
  * **Tài liệu kỹ thuật & Đồ họa**: Chủ trì biên tập báo cáo đồ án, vẽ sơ đồ lớp tổng quát, sơ đồ tuần tự và sơ đồ luồng nghiệp vụ.
  * **Đảm bảo Chất lượng (QA)**: Thực hiện kiểm thử tích hợp giữa giao diện và nghiệp vụ, tối ưu hóa hiệu năng hiển thị và rà soát tính nhất quán của tài liệu.
