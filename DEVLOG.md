...
| D02 | 2026-04-24 | Domain / Trip | Dùng **State Pattern** (`ITripState` + 8 state classes) thay vì switch-case trên `TripStatus` enum | Đảm bảo chỉ chuyển trạng thái hợp lệ; dễ thêm state mới không sửa Trip class; tách biệt behavior theo trạng thái | Mọi transition phải đi qua state object → không thể bypass business rule từ bên ngoài. **Trip dùng State Pattern, Driver dùng State Machine.** |
